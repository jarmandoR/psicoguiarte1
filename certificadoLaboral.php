...
if ($formatofin==1){
	    $pdf->MultiCell(0, 6, utf8_decode('Que '.$nombre.', identificada con la cédula de ciudadanía N.º '.$cedula.' de Bogotá, labora en nuestra empresa desde el '.date("d",$fechadelregi).' de '.date("m",$fechadelregi).' del '.date("Y",$fechadelregi).' con un CONTRATO A TERMINO INDEFINIDO, desempeñando el cargo de '.$cargo.'.'), 0,'J');
}else if($formatofin==2){
...
$pdf->Cell(40, 6, 'Recursos humanos', 'B', 1);

// $pdf->Cell(0, 6, '______________________________ ', 0, 1);
$pdf->Cell(0, 6, 'RECURSOS HUMANOS', 0, 1);

$pdf->Cell(0, 6, 'Recursos humanos', 0, 1);


$pdf->Ln(10);
$pdf->MultiCell(0, 6, 'Puede validar la veracidad de este certificado en el correo: talentohumano@bermudas.com.co o al celular: 316 0110021
', 0,'C');
